...
if not os.path.exists('config.ini'):
    open('config.ini', 'w', encoding="utf8").close()
...
while True:    
    ...
    if delete in yes:
        os.chdir("sales")
        open(f"sales_{volume}{trafficTypeFile}_{cost}p.txt", "w", encoding="utf8").close()
        os.chdir("..")
        break
    elif delete in no:
        break
...
def check(id):
    ...
    return False, None
trafficTypeVisual = " ГБ" if trafficType == "data" else " мин." + ending if trafficType == "voice" else " SMS"
if s == 0 and r == 0:
    k = round((b_p/(s+1+r)), 2)
else:
    k = round((b_p/(s+r)), 2)
    ...
    try:
        for item in data["data"][:20]:
            ...
            if useEmojis in yes:
                ...
                emoji_dict = {"devil": "}⁠:-)", "cool": "B⁠-⁠) ", "cat": "^ｪ^ ", "zipped": ":⁠-⁠! ", "scream": ":⁠-⁠O ", "rich": ":⁠-⁠$ ", "tongue": ":⁠-⁠P ", "bomb": "*~O ", "": "    "}
                for i in range(len(emojis)):
                    try:
                        emojis[i] = emoji_dict[emojis[i]]
                    except KeyError:
        ...
        for element in sellers_array[:20]:
            ...
            if id not in [element[-2] for element in selection_array]:
                ...
                if (element[-1] == 'True' and 0 <= time.time() - timestamp <= 3600):
                    ...
                    f.write(str(element[0]) + ' - Бот. Время покупки: {}'.format(datetime.datetime.now().strftime("%H:%M:%S %d.%m.%Y")) + "\n")
                    b_b += 1; b += 1
                    ...
                    if not result:
                        ...
                        f.write(str(element[0]) + ' - Продавец. Время покупки: {}'.format(datetime.datetime.now().strftime("%H:%M:%S %d.%m.%Y")) + "\n")
                        b_p += 1; b += 1                
                    else:
                        if useEmojis in yes:
